...
<nav>
    <div class="flex justify-between bg-blue-50">
        <div class="flex gap-8  p-2">
            <div class="flex cursor-pointer" onclick="location.href='/'" title="Terug naar home">
                <!-- Logo van je website -->
            ...
            <div>
                <a href="/berichten" class="hover:text-indigo-700 <?= isUri("/berichten/index") ? 'bg-purple-200 rounded p-2' : '' ?>">Berichten</a>
            </div>
        </div>
        <?php if (isLogin()): ?>
            <div class="p-1">
                <div class="rounded bg-purple-200 p-2"><?= user()->name ?></div>
            </div>
        <?php endif ?>
